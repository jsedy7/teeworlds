Zip = {
	...
	OptFind = function (name, required)
		local check = function(option, settings)
			...
			if ExecuteSilent("pkg-config libzip") == 0 then
				option.value = true
				option.use_pkgconfig = true
			elseif ExecuteSilent("libzip-config") > 0 and ExecuteSilent("libzip-config --cflags") == 0 then
				option.value = true
		...
		local apply = function(option, settings)
			if option.use_pkgconfig == true then
				settings.cc.flags:Add("`pkg-config --cflags libzip`")
				settings.link.flags:Add("`pkg-config --libs libzip`")
			elseif option.use_winlib > 0 then
				settings.cc.includes:Add(Zip.basepath .. "/include")
				if option.use_winlib == 32 then
					settings.link.libpath:Add(Zip.basepath .. "/windows/lib32")
				else
					settings.link.libpath:Add(Zip.basepath .. "/windows/lib64")
				end
				settings.link.libs:Add("zip")
			end
		end

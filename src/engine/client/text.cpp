...
public:
	...
	virtual void TextDeferredRenderEx(CTextCursor *pCursor, const char *pText, int Length, CQuadChar* aQuadChar, int QuadCharMaxCount, int* pQuadCharCount, IGraphics::CTextureHandle* pFontTexture)
	{
		...
		*pFontTexture = pSizeData->m_aTextures[0];
		...
		while(pCurrent < pEnd && (pCursor->m_MaxLines < 1 || LineCount <= pCursor->m_MaxLines))
		{
			...
			if(pCursor->m_LineWidth > 0 && !(pCursor->m_Flags&TEXTFLAG_STOP_AT_END))
			{
				...
				TextDeferredRenderEx(&Compare, pCurrent, Wlen, aQuadChar, QuadCharMaxCount, pQuadCharCount, pFontTexture);
				...
				if(Compare.m_X-DrawX > pCursor->m_LineWidth)
				{
					...
					TextDeferredRenderEx(&Cutter, (const char *)pCurrent, Wlen, aQuadChar, QuadCharMaxCount, pQuadCharCount, pFontTexture);
	...
	void TextCalculateRect(CTextCursor *pCursor, const char *pText, int Length, float* pOutRect) // DUCK
	{
		CFont *pFont = pCursor->m_pFont;
		CFontSizeData *pSizeData = NULL;
		...
		// if we don't want to render, we can just skip the first outline pass
		{
			...
			while(pCurrent < pEnd && (pCursor->m_MaxLines < 1 || LineCount <= pCursor->m_MaxLines))
			{
				...
				if(pCursor->m_LineWidth > 0 && !(pCursor->m_Flags&TEXTFLAG_STOP_AT_END))
				{
					...
					Compare.m_Flags &= ~TEXTFLAG_RENDER;
					Compare.m_LineWidth = -1;
					TextCalculateRect(&Compare, pCurrent, Wlen, pOutRect);
					...
					if(Compare.m_X-DrawX > pCursor->m_LineWidth)
					{
						...
						TextCalculateRect(&Cutter, (const char *)pCurrent, Wlen, pOutRect);
						Wlen = Cutter.m_GlyphCount;
						NewLine = 1;
				...
				while(pCurrent < pBatchEnd)
				{
					...
					if(pChr)
					{
						...
						if(pCursor->m_Flags&TEXTFLAG_RENDER)
						{
							float CharX = DrawX+pChr->m_OffsetX*Size;
							float CharY = DrawY+pChr->m_OffsetY*Size;
							float CharW = pChr->m_Width*Size;
							float CharH = pChr->m_Height*Size;
							pOutRect[0] = min(pOutRect[0], CharX);
							pOutRect[1] = min(pOutRect[1], CharY);
							pOutRect[2] = max(pOutRect[2], CharX + CharW);
							pOutRect[3] = max(pOutRect[3], CharY + CharH);
						}
